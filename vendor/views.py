    ...
    if request.method == "POST":
        ...
        if store_id:
            try:
                ...
                # Category is locked after creation — never update it on edit
                store.phone = phone
...
def toggle_store_status(request, store_id):
    """Toggle a store between Active and Inactive."""
    vendor_id = request.session.get("user_id")
    try:
        store = Store.objects.get(id=store_id, vendor_id=vendor_id)
        store.status = not store.status
        store.save()
        state = "activated" if store.status else "deactivated"
        messages.success(request, f'"{store.store_name}" has been {state}.')
    except Store.DoesNotExist:
        messages.error(request, 'Store not found.')
    return redirect("vendor_store")
...
def vendor_events(request):
    ...
    bookings = Booking.objects.filter(vendor_id=vendor_id).select_related('event', 'store', 'store__category')
    ...
    # Build a map of event_id -> booking for this vendor
    booking_by_event = {b.event_id: b for b in bookings}

    # Add access info and photographer flag to each event for template
    events_list = list(events)
    for event in events_list:
        event.has_guest_access = event.id in access_by_event
        if event.has_guest_access:
            event.guest_access_obj = access_by_event[event.id]
        # Show guest credentials only if the booking is from a photographer store
        booking = booking_by_event.get(event.id)
        event.is_photographer_booking = (
            booking is not None and
            'photographer' in booking.store.category.name.lower()
        )
...
def generate_guest_credentials(request, event_id):
    ...
    # Only photographer vendors can generate guest credentials
    if 'photographer' not in booking.store.category.name.lower():
        messages.error(request, 'Guest credentials can only be generated by photographers.')
        return redirect('vendor_events')
    ...
    # Check if credentials already exist for this event+vendor
    existing = EventGuestAccess.objects.filter(event=event, vendor=vendor).first()

    if existing:
        # Regenerate: keep the same guest ID, only update the password
        existing.password = make_password(password)
        existing.is_active = True
        existing.save()
        created = False
    else:
        # First time: generate a new guest ID
        guest_id = generate_guest_id()
        EventGuestAccess.objects.create(
            event=event,
            vendor=vendor,
            guest_id=guest_id,
    ...
    if created:
        messages.success(request, 'Guest credentials generated successfully!')
    else:
        messages.success(request, 'Password regenerated successfully! Guest ID remains the same.')
...
def view_guest_credentials(request, event_id):
    ...
    # Only photographer vendors can view guest credentials
    if 'photographer' not in booking.store.category.name.lower():
        messages.error(request, 'Guest credentials are only available for photographers.')
        return redirect('vendor_events')
...
# Dedicated page for uploading event images (photographers only) — shows event cards
@vendor_required
def upload_event_image(request):
    ...
    # Restrict access to photographer vendors only
    is_photographer = stores.filter(category__name__icontains='photographer').exists()
    if not is_photographer:
        messages.error(request, 'This page is only available for photographers.')
        return redirect('vendor_dashboard')

    # All events this vendor is booked for
    ...
    # Attach photo count to each event
    photo_counts = {
        item['event_id']: item['count']
        for item in GalleryImage.objects
            .filter(vendor=vendor, uploaded_by_guest=False, event_id__in=event_ids)
            .values('event_id')
            .annotate(count=Count('id'))
    }
    events_list = list(events)
    for event in events_list:
        event.photo_count = photo_counts.get(event.id, 0)
...
# Detail page: all photos for one event (upload + delete)
@vendor_required
def event_photos_detail(request, event_id):
    vendor_id = request.session.get('user_id')
    vendor = User.objects.get(id=vendor_id)

    # Verify photographer
    stores = Store.objects.filter(vendor_id=vendor_id)
    if not stores.filter(category__name__icontains='photographer').exists():
        messages.error(request, 'This page is only available for photographers.')
        return redirect('vendor_dashboard')

    # Verify vendor is booked for this event
    if not Booking.objects.filter(vendor_id=vendor_id, event_id=event_id).exists():
        messages.error(request, 'You are not associated with this event.')
    ...
    if request.method == 'POST':
        ...
        if action == 'upload':
            ...
            image_id = request.POST.get('image_id')
            try:
                img = GalleryImage.objects.get(id=image_id, vendor=vendor, event=event)
                img.delete()
    ...
    photos = GalleryImage.objects.filter(event=event, vendor=vendor, uploaded_by_guest=False).order_by('-uploaded_at')

    return render(request, 'vendor/event_photos_detail.html', {
        'event': event,
        'photos': photos,
        'vendor': vendor,
    })
